...
//The individual component to show a user with his role
const AdminRole = ({ id, email, role, avatar_url }) => {
    const { sessionData, changeRole, isAdmin } = useSession()
    //The users can't manage their own role for security and in case the user has brain lag. Imagine the app run out of admins
    ...
    return (
        <div className="AdminRole_container">
            ...
            <div className="AdminRole_info">
                ...
                {infoMessage && (
                    <p className="AdminNote_infoMessage">
                        {infoMessage.status}
                    </p>
                )}
            </div>
